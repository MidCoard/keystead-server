...
@RestController
@RequestMapping("/api/v1/vaults/{fingerprint}/automation-principals")
class AutomationController {
    ...
    @NonNull ResponseEntity<Void> putPrincipal(
        ...
        automation.putPrincipal(principal.getName(), fingerprint, principalId, request);
    ...
    @NonNull ResponseEntity<Void> putKeyPackage(
        ...
        automation.putKeyPackage(principal.getName(), fingerprint, principalId, request);
        return ResponseEntity.status(HttpStatus.CREATED).build();
    }
    ...
    @NonNull AutomationTokenResponse issueToken(
        ...
        return automation.issueToken(principal.getName(), fingerprint, principalId, request);
    }
    ...
    @NonNull List<AutomationTokenSummary> listTokens(
            ...
            @PathVariable @NonNull String fingerprint,
            @PathVariable @NonNull String principalId) {
        return automation.listTokens(principal.getName(), fingerprint, principalId);
    }
    ...
    @NonNull ResponseEntity<Void> revokeToken(
        ...
        automation.revokeToken(principal.getName(), fingerprint, request);
    ...
    @NonNull ResponseEntity<Void> revokeTokenById(
        ...
        automation.revokeTokenById(principal.getName(), fingerprint, principalId, tokenId);
        return ResponseEntity.noContent().build();
    }
    ...
    @NonNull ResponseEntity<Void> revokePrincipal(
            ...
            @PathVariable @NonNull String fingerprint,
            @PathVariable @NonNull String principalId) {
        automation.revokePrincipal(principal.getName(), fingerprint, principalId);
        return ResponseEntity.noContent().build();
    }
